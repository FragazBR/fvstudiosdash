-- ================================
-- CORREÇÃO DE FUNÇÕES - REMOVE E RECRIA
-- Execute este script no Supabase SQL Editor
-- ================================

-- Remover funções existentes que podem ter conflito
DROP FUNCTION IF EXISTS process_website_lead(text,text,text,text,text,text,text,text,text,text,text,text);
DROP FUNCTION IF EXISTS get_agency_metrics();
DROP FUNCTION IF EXISTS create_user_with_profile(text,text,text,text,text,text);
DROP FUNCTION IF EXISTS get_dashboard_projects(integer,text);
DROP FUNCTION IF EXISTS get_user_clients(integer);
DROP FUNCTION IF EXISTS create_agency_after_payment(text,text,text,text,text);

-- Recriar as funções sem conflitos
...
-- Função: process_website_lead (versão simplificada)
...
DECLARE
  ...
  result json;
BEGIN
  -- Tentar inserir na tabela agency_leads se existir
  BEGIN
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- Se a tabela não existir, gerar um ID aleatório
...
BEGIN
  ...
  SELECT 
    ...
    p.description,
    p.status,
    COALESCE(p.priority, 'medium')::text,
    p.budget_total,
    p.start_date,
    p.end_date,
...
CREATE OR REPLACE FUNCTION get_user_clients(p_limit integer)
RETURNS TABLE (
  id uuid,
  name text,
  email text,
...
-- Mensagem de sucesso
SELECT 'Todas as funções foram recriadas com sucesso!' as status;